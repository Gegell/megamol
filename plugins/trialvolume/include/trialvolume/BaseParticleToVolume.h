 ...
 */
class BaseParticleToVolume : public core::Module {
public:
    enum KernelType : int {
...
protected:
    ...
    virtual bool computeVolume(geocalls::MultiParticleDataCall* caller) = 0;

    /**
     * Check if any of the parameters has changed.
     */
    virtual inline bool anythingDirty() const {
        // TODO expand if more parameters are added
        return voxel_size_slot_.IsDirty()
     ...
     */
    virtual inline void resetDirtyFlags() {
        // TODO expand if more parameters are added
